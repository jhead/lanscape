use crate::agent::webrtc::{WebRTCManager, RTCIceCandidate};
...
impl SignalingClient {
    ...
    async fn handle_message(
        ...
        match msg.message_type.as_str() {
            ...
            "offer" => {
                Self::handle_offer(&msg, webrtc, self_id, url, topic).await?;
            }
            "answer" => {
    ...
    async fn create_peer_connection(
        ...
        if should_create_offer {
            ...
            Self::send_relay(url, topic, "offer", peer_id, payload, None).await?;
        }
    ...
    async fn handle_offer(
        ...
        let offer = webrtc::peer_connection::sdp::session_description::RTCSessionDescription {
            sdp: sdp.to_string(),
            sdp_type: webrtc::sdp::Type::from_str(sdp_type)?,
        };
        ...
        Self::send_relay(url, topic, "answer", peer_id, answer_payload, None).await?;
    ...
    async fn handle_ice_candidate(
        ...
        let candidate = RTCIceCandidate::from_str(candidate_str)?;

        if let Err(e) = webrtc.add_ice_candidate(peer_id, candidate).await {
    ...
    async fn send_relay(
        ...
        // This will be handled by the sender channel
        // We need to get the sender from the client
        // For now, we'll need to pass it through or store it
        // This is a limitation - we'll need to refactor to pass sender
        // For now, let's create a helper that the client can use
        todo!("Need to implement send_relay with proper sender access")
    }
    ...
    pub async fn send_ice_candidate(
        ...
        let payload = json!({
            ...
            "sdpMLineIndex": candidate_json.sdp_m_line_index,
        });
